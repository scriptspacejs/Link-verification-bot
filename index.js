    ...
    try {
        ...
        if (interaction.isStringSelectMenu()) {
            if (interaction.customId.startsWith('embed_main_options_') || interaction.customId.startsWith('embed_options_')) {
                ...
                for (const option of selectedOptions) {
                    switch (option) {
                        case 'select_channel':
                            // Filter channels to only show ones the bot can send messages to
                            const availableChannels = interaction.guild.channels.cache
                                .filter(channel => {
                                    ...
                                    return botPermissions && botPermissions.has(['SendMessages', 'EmbedLinks', 'ViewChannel']);
                                })
                                .map(channel => ({
                                    label: `#${channel.name}`,
                                    description: `Send embed to #${channel.name}`,
                                    value: channel.id,
                                    emoji: channel.id === embedData[embedId].targetChannel ? '✅' : '📢'
                                }))
                                .slice(0, 25); // Discord limit

                            if (availableChannels.length === 0) {
                                await interaction.followUp({
                                    content: '❌ **No accessible channels found!** I need "Send Messages" and "Embed Links" permissions in channels to send embeds there.',
                                    ephemeral: true
                                });
                                continue;
                            }
                            ...
                            const currentChannel = interaction.guild.channels.cache.get(embedData[embedId].targetChannel);
                            await interaction.followUp({
                                content: `📡 **Select Target Channel:**\n${currentChannel ? `**Current:** ${currentChannel}` : '**Current:** None selected'}\n✅ = Currently selected channel`,
            ...
            if (interaction.customId.startsWith('send_embed_')) {
                ...
                if (!embedData[embedId].targetChannel) {
                    return interaction.editReply({
                        content: '❌ **No target channel selected!** Please select a channel first using the dropdown menu.'
                    });
                }
                ...
                if (!channel) {
                    return interaction.editReply({
                        content: '❌ **Target channel not found!** The selected channel may have been deleted.'
                    });
                }
                ...
                if (!botPermissions || !botPermissions.has(['SendMessages', 'EmbedLinks'])) {
                    return interaction.editReply({
                        content: `❌ **Missing permissions!** I need "Send Messages" and "Embed Links" permissions in ${channel}.`
                    });
                }
                ...
                try {
                    ...
                    for (let i = 0; i < finalEmbeds.length; i += 10) {
                        ...
                        if (validEmbeds.length > 0) {
                            const sentMessage = await channel.send({ embeds: validEmbeds });
                            sentMessages++;
                    ...
                    if (sentMessages > 0) {
                        await interaction.editReply({
                            content: `🚀 **Embed sent successfully!**\n📍 **Channel:** ${channel}\n📊 **Details:** ${messageLength} characters, ${totalEmbedsSent} embed${totalEmbedsSent > 1 ? 's' : ''}, ${sentMessages} message${sentMessages > 1 ? 's' : ''}\n✅ **Status:** Complete`
                        });
        ...
        if (interaction.isModalSubmit()) {
            ...
            if (interaction.customId.startsWith('image_modal_')) {
                ...
                if (imageUrl) {
                    try {
                        ...
                        const isValidImage = /\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(imageUrl) || 
                                           /^https:\/\/(i\.imgur\.com|cdn\.discordapp\.com|media\.giphy\.com|i\.gyazo\.com|prnt\.sc)/i.test(imageUrl);
                        
                        if (!isValidImage) {
                            return await interaction.reply({
                                content: '❌ **Invalid image URL!** Please use a direct image link ending with .jpg, .png, .gif, .webp, or from supported hosts (imgur, discord, giphy, etc.)',
                                ephemeral: true
                            });
                        }
                        
                        embedData[embedId].image = imageUrl;
                        await interaction.reply({
                            content: `🖼️ **Image added successfully!**\n📎 ${imageUrl}`,
                            ephemeral: true
                        });
                    } catch (error) {
                        return await interaction.reply({
                            content: '❌ **Invalid URL format!** Please enter a valid image URL.',
                            ephemeral: true
                        });
                    }
                } else {
                    embedData[embedId].image = null;
                    await interaction.reply({
                        content: '🖼️ **Image removed from embed**',
                        ephemeral: true
                    });
                }
            }
            ...
            if (interaction.customId.startsWith('video_modal_')) {
                const videoUrl = interaction.fields.getTextInputValue('video_url').trim();
                
                if (videoUrl) {
                    try {
                        new URL(videoUrl);
                        // Accept various video formats and platforms
                        const isValidVideo = /\.(mp4|mov|avi|mkv|webm|m4v)(\?.*)?$/i.test(videoUrl) || 
                                           /^https:\/\/(www\.)?(youtube\.com\/watch\?v=|youtu\.be\/|vimeo\.com\/|streamable\.com\/)/i.test(videoUrl);
                        
                        embedData[embedId].video = videoUrl;
                        await interaction.reply({
                            content: `🎥 **Video added successfully!**\n📎 ${videoUrl}\n${isValidVideo ? '✅ Supported format detected' : '⚠️ URL accepted but may not display properly'}`,
                            ephemeral: true
                        });
                    } catch (error) {
                        return await interaction.reply({
                            content: '❌ **Invalid URL format!** Please enter a valid video URL.',
                            ephemeral: true
                        });
                    }
                } else {
                    embedData[embedId].video = null;
                    await interaction.reply({
                        content: '🎥 **Video removed from embed**',
                        ephemeral: true
                    });
                }
            }
            ...
            if (interaction.customId.startsWith('thumbnail_modal_')) {
                ...
                if (thumbnailUrl) {
                    try {
                        ...
                        // Validate thumbnail URL (similar to image)
                        const isValidThumbnail = /\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(thumbnailUrl) || 
                                               /^https:\/\/(i\.imgur\.com|cdn\.discordapp\.com|media\.giphy\.com|i\.gyazo\.com|prnt\.sc)/i.test(thumbnailUrl);
                        
                        if (!isValidThumbnail) {
                            return await interaction.reply({
                                content: '❌ **Invalid thumbnail URL!** Please use a direct image link ending with .jpg, .png, .gif, .webp, or from supported hosts.',
                                ephemeral: true
                            });
                        }
                        
                        embedData[embedId].thumbnail = thumbnailUrl;
                        await interaction.reply({
                            content: `🖼️ **Thumbnail added successfully!**\n📎 ${thumbnailUrl}`,
                            ephemeral: true
                        });
                    } catch (error) {
                        return await interaction.reply({
                            content: '❌ **Invalid URL format!** Please enter a valid thumbnail URL.',
                            ephemeral: true
                        });
                    }
                } else {
                    embedData[embedId].thumbnail = null;
                    await interaction.reply({
                        content: '🖼️ **Thumbnail removed from embed**',
                        ephemeral: true
                    });
                }
            }
            ...
            if (interaction.customId.startsWith('author_modal_')) {
                ...
                if (authorName) {
                    embedData[embedId].authorName = authorName;
                    
                    if (authorIcon) {
                        try {
                            new URL(authorIcon);
                            const isValidIcon = /\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(authorIcon) || 
                                              /^https:\/\/(i\.imgur\.com|cdn\.discordapp\.com|media\.giphy\.com|i\.gyazo\.com|prnt\.sc)/i.test(authorIcon);
                            
                            if (!isValidIcon) {
                                embedData[embedId].authorIcon = null;
                                await interaction.reply({
                                    content: `👤 **Author set:** ${authorName}\n⚠️ **Invalid icon URL ignored** - please use a direct image link`,
                                    ephemeral: true
                                });
                            } else {
                                embedData[embedId].authorIcon = authorIcon;
                                await interaction.reply({
                                    content: `👤 **Author set:** ${authorName}\n🖼️ **With icon:** ${authorIcon}`,
                                    ephemeral: true
                                });
                            }
                        } catch (error) {
                    ...
                    embedData[embedId].authorName = null;
                    embedData[embedId].authorIcon = null;
                    await interaction.reply({
                        content: '👤 **Author removed from embed**',
                        ephemeral: true
                    });
                }
            }
