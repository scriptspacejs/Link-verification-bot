...
const GITHUB_USERNAME = 'scriptspacejs'; // Replace with your GitHub username
const GITHUB_REPO = 'Link-verification-bot'; // Replace with your repository name
const GITHUB_BRANCH = 'main'; // Replace with your branch name (usually 'main' or 'master')

// Local GIFs folder path
const GIFS_FOLDER = './gifs';

// Load GIFs from local folder and create GitHub URLs from repo root
function loadLocalGifs() {
    try {
        ...
        return gifFiles.map(file => {
            return {
                localPath: path.join(GIFS_FOLDER, file),
                githubUrl: `https://raw.githubusercontent.com/${GITHUB_USERNAME}/${GITHUB_REPO}/${GITHUB_BRANCH}/${file}`,
                fileName: file
            };
        });
    } catch (error) {
        console.error('Error loading local GIFs:', error);
        return [];
    }
}

// Initialize GIFs collection
const cuteGifs = loadLocalGifs();

// Designated cute GIF channel
const CUTE_GIF_CHANNEL = '1377703145941106738';

// Level up GIFs from local folder (celebration moments!)
const levelUpGifNames = [
    '24260-congratulations_1757992257998.gif', // congratulations
    '99097-eatingcake_1757992258328.gif', // eating cake
    '63168-iloveyou_1757992258100.gif', // i love you
    '97113-peachhearts_1757992258272.gif', // peach hearts
    '97411-gomaglowsticks_1757992258292.gif' // goma glow sticks
];

const levelUpGifs = cuteGifs.filter(gif => 
    levelUpGifNames.some(name => gif.fileName === name)
);
...
function getRandomGif() {
    ...
    return randomGif ? randomGif.githubUrl : null;
}
...
async function checkLevelUp(userId, guild, xpGained = 0) {
    ...
    if (newLevel > oldLevel) {
        ...
        if (levelUpChannel) {
            ...
            const gifFileName = getFileNameFromUrl(levelUpGif);
            ...
            const levelUpEmbed = new EmbedBuilder()
                ...
                .setImage(levelUpGif) // Use direct URL
...
client.on('messageCreate', async (message) => {
    ...
    if (isGamingMessage || (containsAtEveryone && hasGamingContext) || hasPromotionContext) {
        try {
            ...
            const tinyWarningGifNames = [
                '66165-peachgomababy_1757992258160.png', // tiny baby peach goma
                '8133-peachgoma_1757992258233.png', // small peach goma
                '48590-peachgoma_1757992258084.png' // small peach goma
            ];

            const tinyWarningStickers = cuteGifs.filter(gif => 
                tinyWarningGifNames.some(name => gif.fileName === name)
            ).map(gif => gif.githubUrl); // Use githubUrl here

            const warningSticker = tinyWarningStickers[Math.floor(Math.random() * tinyWarningStickers.length)];
            const warningFileName = getFileNameFromUrl(warningSticker);
            ...
            const warningEmbed = new EmbedBuilder()
                ...
                .setThumbnail(warningSticker) // Use direct URL
    ...
    if (message.channel.id === CUTE_GIF_CHANNEL && Math.random() < 0.08) { // 8% chance to prevent spam
        try {
            ...
            const tinyStickerNames = [
                '66165-peachgomababy_1757992258160.png', // tiny baby peach goma
                '8133-peachgoma_1757992258233.png', // small peach goma
                '48590-peachgoma_1757992258084.png', // small peach goma
                '88662-kissingyou_1757992258251.png', // small kissing
                '121771-nikkienthusiastic_1757992803790.png', // small nikki
                '174779-nikkibutterfly_1757992803833.png', // small nikki butterfly
                '260537-nikkiserene_1757992803851.png' // small nikki serene
            ];

            const tinyStickers = cuteGifs.filter(gif => 
                tinyStickerNames.some(name => gif.fileName === name)
            ).map(gif => gif.githubUrl); // Use githubUrl here

            const tinySticker = tinyStickers[Math.floor(Math.random() * tinyStickers.length)];
            const fileName = getFileNameFromUrl(tinySticker);

            const stickerEmbed = new EmbedBuilder()
                .setColor(0x000000) // Pure black for cute elegance
                .setThumbnail(tinySticker) // Use direct URL
                .setFooter({ text: '💖 Tiny cute moment!', iconURL: message.author.displayAvatarURL() });

            // Send with direct URL
            await message.reply({ 
                embeds: [stickerEmbed]
            });
