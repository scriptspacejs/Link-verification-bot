...
function getXPRequiredForLevel(level) {
    ...
    if (level <= 1) return 0;
    return Math.floor(1000 * Math.pow(1.1, level - 2));
}
        ...
        if (levelUpChannel) {
            ...
            const levelUpEmbed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '📊 Progress to Next', value: `${progressXP}/${neededXP} XP`, inline: true }
                )
                .setColor(0x000000) // Pure black for elegant level up notifications
                .setImage(getRandomLevelUpGif())
                .setTimestamp()
                .setFooter({ text: '🎊 Keep chatting to gain more XP and level up!' });
...
async function addMessageXP(userId, guild) {
    ...
    console.log(`💰 ${guild.members.cache.get(userId)?.displayName || 'User'} gained ${xpToGain} XP (Total: ${user.totalXP} XP)`);
...
client.on('ready', () => {
    ...
    // Set bot status
    client.user.setActivity('made with love by script', { type: 'PLAYING' });
});

// Function to create embed preview
function createEmbedPreview(data) {
    const embed = new EmbedBuilder()
        .setDescription(data.message + (data.selectedEmojis.length > 0 ? '\n\n' + data.selectedEmojis.join(' ') : ''))
        .setTimestamp();

    // Set embed style based on type
    switch (data.embedType) {
        case 'success':
            embed.setColor(0x00FF00).setTitle('✅ Success');
            break;
        case 'error':
            embed.setColor(0xFF0000).setTitle('❌ Error');
            break;
        case 'warning':
            embed.setColor(0xFFFF00).setTitle('⚠️ Warning');
            break;
        case 'info':
            embed.setColor(0x0099FF).setTitle('ℹ️ Information');
            break;
        case 'cute':
            embed.setColor(0xFF69B4).setTitle('💖 Cute Message');
            break;
        case 'announcement':
            embed.setColor(0x9932CC).setTitle('📢 Announcement');
            break;
        default:
            embed.setColor(0x000000).setTitle('📝 Message');
    }
...
// Handle interactions (dropdowns, buttons, modals)
client.on('interactionCreate', async (interaction) => {
    ...
    if (!embedData[embedId] || embedData[embedId].authorId !== interaction.user.id) {
        if (interaction.user.id !== BOT_OWNER_ID) {
            return interaction.reply({ content: '❌ Only the bot owner can use embed commands!', ephemeral: true });
        }
    }
    ...
    try {
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId.startsWith('send_embed_')) {
                ...
                const finalEmbed = createEmbedPreview(embedData[embedId]);
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId.startsWith('message_modal_')) {
                ...
                await interaction.reply({
                    content: '📝 **Message updated!** Updated preview:',
                    embeds: [previewEmbed],
                    ephemeral: true
                });
            }
...
client.on('messageCreate', async (message) => {
    ...
    // Check for embed command (owner only) - Auto-opens dropdown
    if (message.content.toLowerCase().startsWith('emb ') && message.author.id === BOT_OWNER_ID) {
        const embedMessage = message.content.slice(4).trim();
        if (!embedMessage) {
            return message.reply('❌ Please provide a message after `emb`! Example: `emb Hello World!`');
        }
        ...
        embedData[embedId] = {
            ...
            selectedEmojis: [],
            embedType: 'basic',
            targetChannel: message.channel.id, // Auto-set to current channel
        ...
        const previewEmbed = createEmbedPreview(embedData[embedId]);
        const currentChannel = message.guild.channels.cache.get(embedData[embedId].targetChannel);
